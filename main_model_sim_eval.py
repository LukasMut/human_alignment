...
def evaluate(args) -> None:
    ...
    for i, model_name in tqdm(enumerate(model_cfg.names), desc="Model"):
        ...
        corr_coef = vision.correlate_rdms(rdm_dnn, rdm_humans, correlation='spearman')
        
        if args.verbose:
            print(
                f"\nModel: {model_name}, Family: {family_name}, Zero-shot correlation coefficient: {corr_coef:.4f}\n"
            )
        summary = {
            "model": model_name,
            "zero-shot": corr_coef,
